...
class SheetColumnSelectorDialog:
    ...
    def setup_ui(self):
        ...
        tk.Label(filename_subframe, text="📄 Colonne 'Nom de fichier' :", 
    ...
    def validate(self):
        ...
        if not filename_col:
            messagebox.showwarning("Attention", "Veuillez sélectionner la colonne 'Nom de fichier'!")
            return
...
class IndexationManager:
    ...
    def get_start_index(self, filename):
        ...
        if filename in self.reference_data:
            return self.reference_data[filename] + 1
        
        name_without_ext = Path(filename).stem
        for ref_name, nb_enfants in self.reference_data.items():
            if Path(ref_name).stem == name_without_ext:
                return nb_enfants + 1
...
# ========================================
    ...
    def create_modified_xlsx_exact_positions(self, original_path, output_dir, extracted_files):
        """
        Crée Excel modifié avec feuille récapitulative détaillée
        """
        try:
            original_ext = Path(original_path).suffix
            wb = openpyxl.load_workbook(original_path, keep_vba=(original_ext == '.xlsm'))
            
#            # Supprimer ancienne feuille si existe
#            if "FICHIERS_EXTRAITS" in wb.sheetnames:
#                del wb["FICHIERS_EXTRAITS"]
#            
#            # Créer nouvelle feuille
#            ws = wb.create_sheet("FICHIERS_EXTRAITS", 0)
#            
#            # En-tête
#            ws['A1'] = "FICHIERS INCORPORÉS - RÉCAPITULATIF PAR FEUILLE"
...
#            
#            ws['A2'] = "⚠️ Les objets OLE sont toujours dans les cellules - voir colonne 'Feuille'"
#            ws['A2'].font = Font(italic=True, color="FF6600", size=10)
#            ws.merge_cells('A2:E2')
#            
#            # Colonnes
#            headers = ['Position', 'Feuille', 'Fichier extrait', 'Type', 'Statut']
...
#            
#            # Données
#            for i, item in enumerate(sorted(extracted_files, key=lambda x: x['position']), 1):
#                row = i + 4
...
#                
#                if action == 'keep_empty':
##                    ws[f'C{row}'] = "[Fichier vide - 0 Ko]"
#                    ws[f'C{row}'].font = Font(color="FF9900", bold=True)
##                    ws[f'D{row}'] = "Vide"
#                    ws[f'E{row}'] = "⚠️ Non archivé - objet conservé"
#                    ws[f'E{row}'].font = Font(color="FF9900", bold=True)
#                    ws[f'E{row}'].fill = PatternFill(start_color="FFF4E6", end_color="FFF4E6", fill_type="solid")
#                    
#                elif action == 'keep_unsupported':
#                    file_type = item.get('file_type', '?')
#                    ws[f'C{row}'] = f"[Type non supporté: {file_type}]"
#                    ws[f'C{row}'].font = Font(color="FF9900", bold=True)
#                    ws[f'D{row}'] = file_type
#                    ws[f'E{row}'] = "⚠️ Non supporté SharePoint - objet conservé"
#                    ws[f'E{row}'].font = Font(color="FF9900", bold=True)
#                    ws[f'E{row}'].fill = PatternFill(start_color="FFF4E6", end_color="FFF4E6", fill_type="solid")
#                    
#                else:
#                    extracted_name = item.get('extracted_name', '?')
#                    ws[f'C{row}'] = extracted_name
#                    ws[f'C{row}'].font = Font(bold=True)
#                    ws[f'D{row}'] = Path(extracted_name).suffix
#                    ws[f'E{row}'] = f"✅ Extrait → Voir {extracted_name}"
...
#            
            ...
            self.log(f"  ✅ Feuille FICHIERS_EXTRAITS créée")
